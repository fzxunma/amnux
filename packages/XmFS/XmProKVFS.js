...
class XmProKVFS {
  ...
  async mkdir(path, { recursive = true } = {}) {
    ...
    for (let i = 1; i <= parts.length; i++) {
      ...
      if (e.value) {
        throw new Error(`EEXIST: not directory ${sub} ${JSON.stringify(e)}`);
      }
      ops.push({
        type: "set",
        key,
    ...
    if (ops.length) {
      ...
      ops.forEach((op) =>
        a.check({ key: op.key, versionstamp: null }).set(op.key, op.value)
      );
      await a.commit();
    }
  }
  ...
  async readFile(path) {
    ...
    if (!meta.value || meta.value.type !== "file") {
  ...
  async readTextFile(vfsPath) {
    const contentEntry = await this.kv.get([
      ...this.pathToKey(vfsPath),
      "content",
    ]);
  ...
  async writeFile(vfsPath, data) {
    ...
    if (
      ...
      [...this.autoSavePaths].some((p) =>
        vfsPath.startsWith(p + "/") || vfsPath === p
      )
    ) {
  ...
  async syncFromDisk(dirs) {
    for (const { vfsPath, diskPath, autoSave = false } of dirs) {
      ...
      if (!vfsPathNormalize || vfsPathNormalize === "/") {
        throw new Error("vfsPath cannot be root");
      }
  ...
  async saveToDisk(vfsPath = "/") {
    ...
    for await (const entry of this.kv.list({ prefix })) {
      ...
      if (relPath[relPath.length - 1] === "content") {
        const fileVfsPath = "/" +
          relPath.slice(0, -1).filter(Boolean).join("/");
  ...
  async _saveSingleFileToDisk(vfsPath) {
    ...
    try {
      ...
      await Deno.mkdir(new URL(".", fullDiskPath).pathname, {
        recursive: true,
      });
  ...
  async exists(path) {
    return (await this.kv.get(this.pathToKey(this.normalize(path)))).value !==
      null;
  }
  ...
  fallbackToFsWatch(paths, cache) {
    ...
    for (const vfsPrefix of paths) {
      ...
      console.log(
        `[Hot Update FS] 监听磁盘目录: ${normalizedDiskPath} → ${vfsPrefix}`,
      );
      ...
      (async () => {
        for await (const event of watcher) {
          ...
          for (const absolutePath of event.paths) {
            if (
              !absolutePath.endsWith(".vue") && !absolutePath.endsWith(".js")
            ) {
              continue;
            }
            ...
            console.log(
              `[Hot Update FS] 检测到变化: ${absolutePath} → ${vfsPath}`,
            );
            ...
            try {
              if (event.kind === "remove") {
                ...
                console.log(
                  `[Hot Update FS] 同步成功: ${vfsPath}`,
                  key,
                );
              }
            } catch (err) {
              console.error(
                `[Hot Update FS] 同步失败 ${vfsPath}:`,
                err.message,
              );
            }
          }
